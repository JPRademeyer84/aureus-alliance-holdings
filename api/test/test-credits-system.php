...
echo "<h2>Credits System Test</h2>";

// Set up test session
$_SESSION['user_id'] = '1';

$endpoints = [
    'User Credits' => 'http://localhost/aureus-angel-alliance/api/coupons/index.php?action=user_credits',
    'Credit History' => 'http://localhost/aureus-angel-alliance/api/coupons/index.php?action=credit_history'
];

echo "<h3>Testing Credits API Endpoints:</h3>";
                    ...
                    if (isset($jsonData['success'])) {
                        ...
                        if ($success) {
                            if (isset($jsonData['data'])) {
                                $data = $jsonData['data'];
                                if (is_array($data)) {
                                    if (isset($data['total_credits'])) {
                                        echo "Credits: Total $" . $data['total_credits'] . ", Available $" . $data['available_credits'];
                                    } else {
                                        echo "History: " . count($data) . " transactions";
                                    }
                                } else {
...
try {
    ...
    foreach ($tables as $table) {
        ...
        if ($exists) {
            ...
            echo "<p style='color: blue; margin-left: 20px;'>Columns: " . implode(', ', $columnNames) . "</p>";
        } else {
...
echo "<h3>Credits System Status:</h3>";
echo "<div style='background: #f5f5f5; padding: 15px; border-radius: 5px;'>";
echo "<h4>✅ Fixed Issues:</h4>";
echo "<ul>";
echo "<li><strong>CouponRedemption:</strong> Fixed relative path to full URL</li>";
echo "<li><strong>PurchaseDialog:</strong> Fixed relative path to full URL</li>";
echo "<li><strong>Coupons API:</strong> Fixed CORS headers to use proper configuration</li>";
echo "<li><strong>Database Tables:</strong> Created missing user_credits, credit_transactions, and nft_coupons tables</li>";
echo "<li><strong>Available Credits Column:</strong> Added generated column for available_credits calculation</li>";
echo "<li><strong>Error Handling:</strong> Added proper success/error response handling</li>";
echo "</ul>";

echo "<h4>🎯 Expected Results:</h4>";
echo "<ul>";
echo "<li>No more 'Error fetching credit history' errors</li>";
echo "<li>No more 'Failed to fetch credits' errors</li>";
echo "<li>No more database column not found errors</li>";
echo "<li>Proper CORS handling for all ports</li>";
echo "<li>Clean JSON responses without PHP errors</li>";
echo "<li>Functional credit system with balance tracking</li>";
echo "</ul>";
echo "</div>";
